...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!isHost) return send(req, res, 403, { ok: false, error: "Only the host can cancel this session" });

    // Cancel it (do NOT delete records; just hide from lists)
    ...
    console.error("rooms-host-cancel error:", err);
