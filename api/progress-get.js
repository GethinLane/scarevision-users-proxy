...
export default async function handler(req, res) {
  ...
  // Use POST (simple + consistent + avoids odd caching)
  ...
  try {
    ...
    const table = process.env.AIRTABLE_USERS_TABLE; // you set this to tblx3kkRrg37FnLSJ

    if (!token || !baseId || !table) {
      return send(req, res, 500, { ok: false, error: "Server not configured" });
    }

    // Find user record by SquarespaceUserId
