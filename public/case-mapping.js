...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const CASE_TSV = String.raw`1	New presentation of undifferentiated disease, Urgent and Unscheduled care	"Ear, Nose and Throat, Speech and Hearing"
...
351	Investigation / Results, New presentation of undifferentiated disease	Respiratory Health, Haematology`;
  ...
  // Extract first comma-separated token from a field, respecting quotes.
  function firstToken(s){
    s = String(s ?? "").trim();
    if (!s) return "";
    let i = 0, inQ = false, out = "";
    while (i < s.length){
      const ch = s[i];
  ...
  // Extra topic aliases you *actually have in your data* (capitalisation / shorter labels).
  ...
  function canonTopic(raw){
    const first = firstToken(raw);
    const k = normKey(first);
    if (TOPIC_ALIASES.has(k)) return TOPIC_ALIASES.get(k);
    return TOPIC_LOOKUP.get(k) || TOPIC_LOOKUP.get(k.replace(/,/g, "")) || first;
  }
  ...
  function canonGroup(raw){
    ...
    // Domain alias you have in your data (capitalisation mismatch):
    if (k === normKey("Urgent and Unscheduled care")) return "Urgent and unscheduled care";

    return GROUP_LOOKUP.get(k) || GROUP_LOOKUP.get(k.replace(/,/g, "")) || first;
  }
  ...
  function parseCaseTsv(tsv){
    ...
    for (const line of lines){
      ...
      const domainRaw = cols[1];
      const topicRaw  = cols[2];

      out.push({
        id,
        group: canonGroup(domainRaw),
        topic: canonTopic(topicRaw)
      });
    }
    return out;
  }
