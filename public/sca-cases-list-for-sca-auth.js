...
//   1. waitForSCAProgress polls for SCAProgress.getProgress (not .init)
//   2. loadProgressOnce calls SCAProgress.getProgress() (not .init())
//   3. Click handler retry uses SCAAuth.getToken() (not SCAProgress.init())
...
`;
  ...
  // ✅ CHANGE 1: Poll for SCAProgress.getProgress instead of .init
  function waitForSCAProgress(maxMs = 5000, intervalMs = 100) {
    return new Promise((resolve) => {
      const start = Date.now();
      const t = setInterval(() => {
        if (window.SCAProgress?.getProgress) {
          clearInterval(t);
          resolve(true);
          return;
        }
  ...
  // ✅ CHANGE 2: Use SCAProgress.getProgress() instead of SCAProgress.init()
  ...
  function writeCasesCache(records, sig) {
    try {
      localStorage.setItem(
        CASES_CACHE_KEY,
        JSON.stringify({ timestamp: Date.now(), sig, data: records })
      );
    } catch {}
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    try {
      ...
      // ✅ CHANGE 3: Retry via SCAAuth.getToken() instead of SCAProgress.init()
      try {
